...
install_load("readr", "tidyverse", "stringr", "readxl","ggplot2", "plotly","lubridate", "data.table","bit64")
```
#Functions
```{r}
...
new_data_Zulassung <- data_Zulassung %>%
  ...
  select(ID_Fahrzeug,Gemeinden,Zulassung)%>% # Select relevant columns
...
data_fahrzeugZulassung <- left_join(data_OEM,new_data_Zulassung,by="ID_Fahrzeug_Nummer")%>%# Joining cars with Zulassung
  rename(ID_Fahrzeug = ID_Fahrzeug.x)
data_fahrzeugZulassung$ID_Fahrzeug.y <- NULL
