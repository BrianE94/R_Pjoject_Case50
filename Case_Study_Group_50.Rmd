...
#Komponente K1DI1
data_Komponente_K1DI1 <- read_csv("./Data/Komponente/Komponente_K1DI1.csv", col_type=cols(ID_Motor.y=col_character(), Produktionsdatum.y=col_date(format=""), Herstellernummer.y=col_character(),Werksnummer.y=col_character(), Fehlerhaft.y = col_logical(), Fehlerhaft_Datum.y = col_date(format=""), Fehlerhaft_Fahrleistung.y = col_character(),ID_Motor= col_character(),Produktionsdatum=col_date(format=""),Herstellernummer=col_integer(),Werksnummer=col_integer()))# Import data
...
```{r}
# Für die Case Stud nicht relevant
...
new_data_fahrzeug_OEM1_Typ11 <- data_fahrzeug_OEM1_Typ11 %>%
  select(ID_Fahrzeug,Produktionsdatum,Herstellernummer,Werksnummer)%>%
...
new_data_Komponente_K1DI1 <- data_Komponente_K1DI1%>%
  select(ID_Motor.x,Produktionsdatum.x,Herstellernummer.x,Werksnummer.x,ID_Motor.y,Produktionsdatum.y,
         Herstellernummer.y,Werksnummer.y,ID_Motor,Produktionsdatum,Herstellernummer,Werksnummer)%>%
  filter(between(Produktionsdatum, as.Date("2008-03-01"), as.Date("2012-05-11")) | between(Produktionsdatum.x,
        as.Date("2008-03-01"), as.Date("2012-05-11")) | between(Produktionsdatum, as.Date("2008-03-01"),  as.Date("2012-05-11")))
...
data_bestandteile_OEM1 <- bind_rows(new_data_bestandteile_oem1_typ11, 
  ...
  mutate(ID_Fahrzeug_Nummer= as.integer(str_replace_all(ID_Fahrzeug,"-","")))# Get integer value for later join
...
# Left_join die richtige Wahl oder Right join
