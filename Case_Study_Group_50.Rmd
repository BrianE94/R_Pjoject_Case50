...
The intention of this project is to find vehicles of "Type11" and "Type12" for a **recall action** which have installed the faulty diesel engine "K1DI1". The defective engines were produced from 1 March 2008 to 11 May 2012.
The recall action will start in the Hamburg area, as the first diesel bans come into force there. Also, critical cities and communities with a certain amount of registered, affected cars should be identified. 
...
Komponente_K1DI1  | Geodaten_Gemeinden_v1.2_2017-08-22_TrR
...
col_types = cols(ID_Motor.y=col_character(), Produktionsdatum.y=col_date(format = ""), Herstellernummer.y=col_integer(), Werksnummer.y=col_integer(), Fehlerhaft_Datum.y=col_date(format = ""), Fehlerhaft_Fahrleistung.y=col_integer(), ID_Motor=col_character(), Produktionsdatum=col_date(format = ""), Herstellernummer=col_integer(), Werksnummer=col_integer(),Fehlerhaft_Datum=col_date(format = ""), Fehlerhaft_Fahrleistung=col_integer()))
```
...
data_bundesland_plz_ort <- data_bundesland_plz_ort%>% #import data
  ...
  add_row(Bundesland="Sachsen",PLZ="08344",Ort="Gruenhain Beierfeld") # add data to the data set

data_bundesland_plz_ort$Ort <- removeNumbers(data_bundesland_plz_ort$Ort)# remove unnecessary numbers, for more in Information: https://stackoverflow.com/questions/13590139/remove-numbers-from-alphanumeric-characters
...
data_Geodaten_Gemeinden<-data_Geodaten_Gemeinden%>%
  rename(Ort=Gemeinde,PLZ= Postleitzahl)%>%# Rename column for consistency
  add_row(PLZ="87637",Ort="Seeg",Laengengrad=10.6104157,Breitengrad=47.6542215)# Add informtion of place "Seeg"
...
data_vehicle <- left_join(new_data_komponente_k1di1, data_bestandteile_OEM1, by="ID_Motor")%>%# join the data
  drop_na()%>% #delete NA
  distinct()# delite duplicates
...
* To the resulting data set, another column is added, which shows the distance between the city center of Hamburg and any location in the data set.
...
data_Ort2 <- left_join(data_Geodaten_Gemeinden,data_bundesland_plz_ort, by="PLZ")%>%
  rename(Ort=Ort.x)%>%# Rname column
  distinct(Ort, .keep_all=TRUE)# distinct duplicate rows
...
# Adding the total number of affected cars in a city/community                                  
number_towns <- final_data_Group_50%>%  
  count(Ort) # count the number of cties to get the total number
final_data_Group_50 <- left_join(final_data_Group_50,number_towns,by="Ort")%>% #Join total number to the final data set
  distinct(ID_Fahrzeug, .keep_all=TRUE)# distinct the duplicated rows
...
kable(graph_final_data_Group_50,caption="Summary: Final Dataset")
```


# Evaluation

After the data was prepared and merged into a final data set, a Shiny-App was developed to evaluate the data.

## Criteria for the Shiny-App
...
First, the shiny package was loaded and with install.load, all further required R packages were installed and loaded. Afterwards, the data set final_data_Group_50, which was prepared in the steps before, was loaded.


### User Interface (UI)

Then, the UI was programmed. As required, the font was changed.
To adjust the layout to the color "grass-green" it needed to be determined which color or color code was meant by this color name as there were several options. It was decided to use the color code **#698b47** after https://www.britishstandardcolour.com/ or https://www.w3schools.com/colors/colors_british.asp .

While designing the UI, the title of the App as well as the input sliders were colored in the color "Grass Green". The shinytheme "sandstone" was chosen as the App theme because it harmonizes well with the corporate color.

The Layout is divided into a sidebarPanel and a mainPanel.

The **sidebarPanel** contains adjustable inputs.
Since the user is supposed to be able to choose a number of radii, a selectInput was used. Here, the user can choose between 1 to 6 radii.
Depending on the number of radii chosen, there has to be a variable number of inputSliders to choose the radius for each perimeter. Therefore, conditionalPanels depending on the given selectInput were added.
Moreover, a sliderInput was added so that the user can assign a critical amount of affected vehicles.

![*sidebarPanel, color accents in "grass-green"*](https://64.media.tumblr.com/31271117f5e7076502540d9a2de28bd9/6f3e139bb8a6e69e-d3/s640x960/61918ef790eaa10b620a952e782385baff6be772.jpg){height=25%, width=25%}
...
In the server, outputs are created and the requested visualizations are generated. Several reactive functions ensure that all outputs are updated whenever inputs are changed.

#### Map

The **map** ist divided into the two groups **Clustered Markers** and **Cities of Interest**. If not interested in one of the groups, the user can hide it and focus on the information he needs. A golden marker is set to the city center of Hamburg because this is the reference point for the analysis.
...
By clicking on the number of a cluster, the map zooms into the borders of the cluster. When the maximum zoom is reached, clicking on the number of the cluster causes blue markers for each vehicle to pop up. When clicking on these markers, there is a popup containing information about the Vehicle ID of the affected vehicle, when the faulty engine was produced and distance of the city/community from the city center of Hamburg.



![*Blue Markers in one location*](https://64.media.tumblr.com/105bc118144af4d41dda099281bd9e9d/ce071781b55eae41-a6/s1280x1920/1ce7a42c3fcb393ea14d6117fda5da7bd7b67d63.jpg){height=50%, width=50%}

![*Popup containing relevant information*](https://64.media.tumblr.com/af4c83fc16df74d529459b90006882b4/ce071781b55eae41-1a/s1280x1920/ffec34028454c2f42fd86545554b6805b6dd5045.jpg){height=50%, width=50%}


In the **Cities of Interest** group, cities/communities are highlighted with a red marker if the critical number of affected vehicles specified by the user is reached or exceeded. When the user hovers over the marker with the cursor, a label is displayed which provides information on how many vehicles are affected in this city.

![*Cities of Interest, the critical amount is the default value of 5000 vehicles. Number of affected vehicles is displayed for Dresden*](https://64.media.tumblr.com/d64ad4d9688700569c5c7099b7ef2c1b/9174b12a8e613924-5f/s2048x3072/baf7aacb88647f8f5686c972f4d6b26e9120307a.png)

The sliderInput which defines the critical number ranges from 500 to 7500 and can be increased or decreased in steps of one hundred. This facilitates the user to select appropriate values. The default is set to 5000.

![*Cities with at least 500 affected vehicles*](https://64.media.tumblr.com/85fe3c32a56261e238bc62eb547654da/9174b12a8e613924-a8/s2048x3072/cce2484fcf1a8ef542f900dacb6d57182f1dd732.jpg)

There are 94 cities/communities with more than 500 affected vehicles. Displaying this many markers does not give a very clear overview which is why this is chosen to be the lowest critical number the user can select.


![*Cities with at least 7500 affected vehicles*](https://64.media.tumblr.com/4045665e0978569c28b5bfe22cfb08d1/9174b12a8e613924-12/s2048x3072/17ad22060ce3e897fe8acc6aed367d63d119898b.jpg)

There are only 4 cities with at least 7500 affected vehicles. This critical number is chosen as the highest slider value as it makes little sense to filter any further.
...
The title of the plot, the legend and the axis titles are colored in grass-green.
...
There are problems with the x-axis labeling because the plot is a histogram. These problems occur in case one, two, or six radii are selected. Still, the plot should be understandable for the user.
...
The **table** supports the barplot by showing the number of affected vehicles within each state for every perimeter. Also, it shows the total number of affected vehicles within each perimeter in the "Sum" row which is tinted in the corporate color.

Sorting of rows has been disabled, as the cases in radius 1 shall determine the order of States. Pagination has been disabled so all information can be found on one page. Rows are not numbered as this would be unnecessary.

![*Table supporting the barplot*](https://64.media.tumblr.com/178eb1228da7603a49a4b66fd55fcee9/81ccbcdac813a100-95/s2048x3072/469d29810afb27db8b856e11f0bd32ee344f9f44.jpg)

#### Basic Dataset

The **basic dataset** proves all visualizations. The attributes displayed are all considered to be of relevance. Since the table header of the final data set  is in German, it is manually renamed before being displayed to the user.
...
### Problems with the Shiny-App

The **final data set contains more than 500000 affected vehicles**. Thus, the **processing effort** for the computer is **enormously high**, which is why the app sometimes crashes, especially when multiple radii are selected. Hence, a powerful computer is needed to run and use the app without problems.

In the App-code in line 24, the data set used by the App is defined. If the App crashes when all data is used, the pipe operator and the head function can be uncommented (and thereby enabled) to work with a smaller data set. The head function can also be replaced by the tail function to look at the data at the bottom of the data set.

# Results

## Hamburg

As the first driving bans for diesel vehicles were introduced in Hamburg, the Hamburg area is analysed in more detail. The city of Hamburg itself has no cases of affected vehicles. If a radius of 25km around the city center of Hamburg is being searched, 3345 cases can be found. These cases spread in the German states Niedersachsen and Schleswig-Holstein.
...
When the critical number of affected vehicles is set to 5000, 8 cities are highlighted.

![*8 Cities with at least 5000 affected vehicles*](https://64.media.tumblr.com/c45d5d3890048a9db266637774248344/64cb2ae8ccf674b5-99/s2048x3072/af9518e288e14d9be468133bea92c93680aa1926.jpg)

These cities are:

City | Affected Vehicles
------------- | -------------
Köln | 13351
Dortmund  | 8312
Münster | 8053
...
For furhter information about highly affected cities for a smaller critical number of faulty vehicles, the Shiny-App can be used.