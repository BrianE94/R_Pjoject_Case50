...
author: "Long Dinh, Vanessa Schweitzer, Jonas Rieger, Andre Düding, Brian Eiffert"
date: "16 07 2020"
output: 
  html_document:
    theme: paper
...
data_fahrzeug_OEM1_Typ12 <- read_csv("./Data/Fahrzeug/Fahrzeuge_OEM1_Typ11.csv",col_types = cols(X1= col_integer(), ID_Fahrzeug=col_character(), Produktionsdatum= col_date(format = ""), Herstellnumer= col_integer(), Fehlerhaft=col_skip(), Fehlerhaft_Datum=col_character(), Fehlerhaft_Fahrleistung=col_integer()))# Import data
...
# Tidy
In this section the data sets are tidy uped. Here unessesary columns are deleted, the format gets updated and additional information is added. 
## Tidy vehicel data sets
```{r}
...
data_komponente_k1di1 <- data_komponente_k1di1 %>%
  select(ID_Motor.x,ID_Motor.y,ID_Motor,Produktionsdatum.x,Produktionsdatum.y,Produktionsdatum)%>%
  gather('ID_Motor.x','ID_Motor.y','ID_Motor', key = "Motoren",value="ID_Motor_K1DI1", na.rm=TRUE) %>%
  gather('Produktionsdatum.x','Produktionsdatum.y','Produktionsdatum', key = "Daten",value="Produktionsdatum_K1DI1", na.rm=TRUE)%>%
  select(ID_Motor_K1DI1,Produktionsdatum_K1DI1)%>%
  rename(ID_Motor=ID_Motor_K1DI1, Produktionsdatum=Produktionsdatum_K1DI1)
```
## Tidy gegraphical data sets
```{r}
...
data_komponente <- left_join(new_data_komponente_k1di1, data_bestandteile_OEM1, by="ID_Motor")%>%
  drop_na()%>%
  distinct()
...
# Final Dataset with Komponente
final_data_Group_50_K <- left_join(data_komponente,data_Ort,by="ID_Fahrzeug")
number_towns_2 <- final_data_Group_50_K%>%  
  count(Ort)
final_data_Group_50_K <- left_join(final_data_Group_50_K,number_towns_2,by="Ort")%>%
  distinct(ID_Fahrzeug, .keep_all=TRUE)

save(final_data_Group_50_K, file="final_data_Group_50_K.Rda")
